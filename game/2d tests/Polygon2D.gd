...
func get_corner_points():
	for i in range(get_polygon().size()):
		...
		if i > 0:
			var prev_point = get_polygon()[i-1]
			
			#print("Prev point " + str(prev_point.x) + ", " + str(prev_point.y))
			prev_points.append(prev_point)
		
		if i < get_polygon().size()-1:
			var ahead_point = get_polygon()[i+1]
...
func get_tangent(i,j):
	...
	var tang = (get_polygon()[i]-corners[j]).tangent()
...
func _draw():
	#test
	#draw_line(get_polygon()[1], vectors[0], Color(0,1,0))
